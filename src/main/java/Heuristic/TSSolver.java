...
import Data.ManejadorParametros;
import Heuristic.TabuSearch.ModoPrint;
...
public class TSSolver {
    ...
    *****************/
    ...
    private static String getFileNameOUT(Parametros params, Boolean masivo) {
        ...
        fileName += "_datos_nT" + params.nT + "_nL" + params.nL;
        if (!masivo) {
            fileName += "_" + params.configArchivo;
        }
        fileName += ".out";

        LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMddHHmmss");
        String outFile = "api_" + now.format(formatter) + fileName;

        return outFile;
    }

    public static String ejecucionMasiva(ManejadorParametros mp, Parametros params) throws Exception {
        String fileName = getFileNameOUT(params, true);

        String output = params.outPath + fileName;
    ...
    ***/
    ...
    public static String apiMasterEjecucion(RunParameters runParams) throws Exception {
        /*** Leer archivo DAT y configuracion ***/
        ManejadorParametros mp = new ManejadorParametros(runParams);
        ...
        if (params.esMasivo == 1) {
            ...
            for (int i = init_config; i <= 10; i ++) {
                ...
                output += ejecucionMasiva(mp, params);
