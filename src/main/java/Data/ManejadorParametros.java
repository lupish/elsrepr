...
import com.vidal.elsrepr.RunHeuristic.RunParameters;

import Heuristic.Parametros;
import Heuristic.TSExcepcion;
...
public class ManejadorParametros {
    ...
    *****************/
    ...
    public ManejadorParametros() {
        ...
        String paramRoot = System.getProperty("user.dir") + "\\src\\main\\java\\Data\\paramModelo.properties";
    ...
    public ManejadorParametros(RunParameters runParams) {
        InputStream configInput;
        Properties config = new Properties();
        ...
        try {
            ...
            // cargar parametros segun lo recibido en el run
            int nT = runParams.getCantPeriodos();
            int nL = runParams.getCantClientes();
            params = new Parametros(nL, nT);
            params.setCantPeriodos(nT);
            params.setCantClientes(nL);
            params.version = runParams.getVersion();
            params.cantItMax = runParams.getCantIteraciones();
            
            // tasas
            params.tasas = 0;
            params.tasas_valor = 0;
            if (runParams.getTasaRecovery() > 0) {
                params.tasas = 1;
                params.tasas_valor = runParams.getTasaRecovery();
            }

            // saltos
            params.saltos = 0;
            params.saltosMAX = 0;
            if (runParams.getCantSaltos() > 0) {
                params.saltos = 1;
                params.saltosMAX = runParams.getCantSaltos();
            }
            params.saltos_swap_odd = runParams.getEstrategiaSaltos();

            // archivo
            String pathFolder = "";
            params.esMasivo = runParams.getEsMasivo();
            if (params.esMasivo == 1) {
                // archivo por defecto
                pathFolder = runParams.getCarpetaEntrada() + "config1\\";
                String nomArchivo = "datos_nT" + nT + "_nL" + nL + "_AAAAA.dat";
                params.setDatArchivo(pathFolder, nomArchivo);
                params.pathArchivo = pathFolder;
                params.nomArchivo = nomArchivo;
                params.all_configs = 1;
                params.all_configs_path = runParams.getCarpetaEntrada();
                params.init_config = 1;
            } else {
                System.out.println(runParams.getConfigArchivo());
                String nomArchivo = "datos_nT" + nT + "_nL" + nL + "_" + runParams.getConfigArchivo() + ".dat";
                pathFolder = runParams.getCarpetaEntrada() + "config" + runParams.getConfig() + "\\";
                params.setDatArchivo(pathFolder, nomArchivo);
                params.pathArchivo = pathFolder;
                params.nomArchivo = nomArchivo;
                params.all_configs = 0;
                params.configArchivo = runParams.getConfigArchivo();
            }
            
            params.outPath = pathFolder + "OUT\\";


            // cargar el resto de params
            params.setTamListaTabu(Integer.parseInt(config.getProperty("tamListaTabu")));
            params.huL_pond = Integer.parseInt(config.getProperty("huL_pond"));
            params.v3_v2it = Integer.parseInt(config.getProperty("v3_v2it"));
